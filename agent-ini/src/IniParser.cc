				...
				if (open_sections.empty())
					...
					if (!global_values)
					    scanner_error ("%s: values at the top level not allowed.", key.c_str ());
                                        ...
                                        {
					    ini.initValue(key, shell_quoted_value ? YaST::agent_ini::unquote(val) : val, comment, i);
                                        }
				    }
				else
				    {
					open_sections.front()->initValue(key, shell_quoted_value ? YaST::agent_ini::unquote(val) : val, comment, i);
				    }
				comment = "";
			    }
		    }
