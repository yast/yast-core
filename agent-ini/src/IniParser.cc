			...
			if (open_sections.empty ())
				...
				if (!global_values)
				    scanner_error ("%s: values at the top level not allowed.", key.c_str ());
                                ...
                                {
				    ini.initValue (key, unquote( val), comment, matched_by);
                                }
			    }
			else 
                        {
			    open_sections.front()->initValue(key, unquote( val), comment, matched_by);
			}
			comment = "";
		    }
		    else
				...
				if (open_sections.empty())
					...
					if (!global_values)
					    scanner_error ("%s: values at the top level not allowed.", key.c_str ());
                                        ...
                                        {
					    ini.initValue (key, unquote( val), comment, i);
                                        }
				    }
				else
				    {
					open_sections.front()->initValue(key, unquote( val), comment, i);
				    }
				comment = "";
			    }
		    }
