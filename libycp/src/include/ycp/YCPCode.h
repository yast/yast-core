...
/**
 * @short Wrapper for Redo as YCPValue
 *
 * YCPRedo is used as a result value in YCode->evaluate
 * in order to distinguish a normal statement/block (returning
 * a value (return <expr>) or YCPVoid  (return)) from a 'break'
 *
 */
class YCPRedoRep : public YCPValueRep
{
protected:
    friend class YCPRedo;
...
public:
    /**
     * Compares two YRedos for equality, greaterness or smallerness.
     * 
     */
    YCPOrder compare (const YCPRedo &v) const { return YO_EQUAL; }

    /**
     * Returns an ASCII representation of the YRedo.
     *   Executing 'break;' always results in YCPNull() 
     */
    string toString() const { return "nil"; }
     ...
     * Returns YT_BREAK. See @ref YCPValueRep#type.
     */
    YCPValueType valuetype() const { return YT_REDO; }

};

/**
 * @short Wrapper for YCPRedoRep
 * This class realizes an automatic memory management
 * via @ref YCPElement. Access the functionality of YCPRedoRep
 * with the arrow operator. See @ref YCPRedoRep.
 */
class YCPRedo : public YCPValue
{
    DEF_COMMON(Redo, Value);
public:
    YCPRedo() : YCPValue (new YCPRedoRep ()) {}
    YCPRedo (std::istream & str) : YCPValue (new YCPRedoRep ()) {}
};
